...
export async function analyzeJobOffer(data: JobInputData): Promise<AnalysisResult> {
  return withRetry(async () => {
    const ai = getAI();
    const systemInstruction = `You are an elite Cyber Security and HR Fraud Investigator. Your goal is to protect students from predatory internship scams and fake jobs. 
    Pay EXTREME attention to these "Red Flags":
    1. UNPAID TECHNICAL ROLES: Unpaid SDE or Developer roles for for-profit companies are often deceptive/scams.
    2. PLAGIARIZED 'ABOUT US' SECTIONS: Check if the 'About Company' description is copied from a famous company while pretending to be a smaller one.
    3. GENERIC BUTTONS: Offers that tell you to click a button like 'Accept Offer' without a formal, legally-binding contract PDF are highly suspicious.
    4. MASS RECRUITMENT: Look for generic templates like 'Dear Candidate' or generic names.
    5. SUSPICIOUS DOMAINS: If the recruiter uses a public domain (Gmail, Outlook) or a domain that doesn't match the company exactly.
    6. INTERNSHIP FARMS: Identify patterns of companies that hire thousands of "unpaid interns" for data mining or social media marketing.
    
    If the offer is unpaid and for a technical role, increase the risk_rate significantly (above 70%).
    Return JSON only.`;

    const textPrompt = `Analyze this job offer for fraud: 
    TITLE: ${data.title}
    COMPANY: ${data.company}
    ABOUT COMPANY: ${data.about_company || 'Not provided'}
    STIPEND: ${data.salary}
    DESC: ${data.description}
    EMAIL: ${data.email}
    URL: ${data.website}
    
    If an image is attached, OCR it and look specifically for "Unpaid", "Accept Offer", or "Confidentiality" clauses.`;
...
export async function chatWithAssistant(messages: ChatMessage[]): Promise<string> {
  return withRetry(async () => {
    ...
    Specifically warn users about "Unpaid SDE Internships" and "Internship Farms".`;
    const response = await ai.models.generateContent({
      model: "gemini-3-flash-preview",
