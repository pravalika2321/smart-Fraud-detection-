...
const Navbar: React.FC<NavbarProps> = ({ currentView, onNavigate, user, onLogout }) => {
  ...
  const links = [
    ...
    { name: 'Live Mock', view: 'interview-practice' },
          ...
          <div className="hidden md:flex items-center space-x-4 lg:space-x-6">
            {links.map((link) => (
              <button
                ...
                className={`text-[11px] lg:text-sm font-bold transition-colors whitespace-nowrap ${
            ...
            {!user ? (
              <button onClick={() => onNavigate('login')} className="bg-blue-600 text-white px-4 lg:px-5 py-2 rounded-xl text-xs lg:text-sm font-black hover:bg-blue-700 transition shadow-lg shadow-blue-200 uppercase tracking-tighter">Login</button>
            ) : (
              <div className="relative">
                <button onClick={() => setProfileMenuOpen(!profileMenuOpen)} className="flex items-center space-x-2 bg-slate-100 px-3 py-1.5 rounded-xl hover:bg-slate-200 transition border border-slate-200">
                  ...
                  <span className="text-xs font-bold text-slate-700 max-w-[100px] lg:max-w-[120px] truncate">{user.username}</span>
                ...
                {profileMenuOpen && (
                  <div className="absolute right-0 mt-2 w-64 bg-white rounded-2xl shadow-2xl border border-slate-100 overflow-hidden py-1 animate-in zoom-in-95 duration-200 origin-top-right">
                    ...
                    <div className="p-1.5">
                      ...
                      <button onClick={() => { onNavigate('interview-practice'); setProfileMenuOpen(false); }} className="w-full text-left px-4 py-2.5 text-sm text-slate-700 hover:bg-slate-50 rounded-xl transition-colors flex items-center">
                        <i className="fas fa-headset mr-3 text-emerald-600 w-4"></i> 
                        AI Live Mock
                      </button>
        ...
        <div className="md:hidden glass-morphism border-t border-slate-100 p-4 space-y-2 animate-in slide-in-from-top-2">
          ...
          {!user ? (
            ...
            <>
              ...
              <button onClick={() => { onNavigate('interview-practice'); setIsOpen(false); }} className="w-full text-left px-4 py-3 text-sm font-bold text-slate-600">AI Live Mock</button>
