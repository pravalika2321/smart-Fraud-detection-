...
const InputModule: React.FC<InputModuleProps> = ({ onAnalyze }) => {
  ...
  const getInputClass = (value: string) => {
    const base = "w-full px-4 py-3 border rounded-xl focus:ring-2 focus:ring-blue-500 focus:outline-none transition-all duration-300 font-medium text-slate-800 placeholder-slate-400 shadow-sm text-sm";
    const bgColor = "bg-white border-slate-200";
    return `${base} ${bgColor}`;
  };
  ...
  return (
    <div className="bg-white rounded-2xl shadow-2xl border border-slate-100 overflow-hidden transition-all duration-500 mx-auto w-full">
      ...
      <form onSubmit={handleSubmit} className="p-4 sm:p-6 md:p-8">
        <div className="mb-6 p-4 bg-amber-50 rounded-xl border border-amber-100 flex items-start">
           <i className="fas fa-circle-info text-amber-500 mt-1 mr-3"></i>
           <p className="text-xs text-amber-800 font-medium">
             <span className="font-bold">Pro Tip:</span> If the internship is <span className="underline">unpaid</span> for an SDE/Technical role, make sure to include that in the salary field or description. This is a major scam indicator.
           </p>
        </div>
        ...
        {activeTab === 'manual' && (
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-8 animate-in fade-in duration-500">
            ...
            <div className="space-y-1.5">
              <label className="text-xs sm:text-sm font-bold text-slate-800">Salary / Stipend (Mark 'Unpaid' if applicable)</label>
              <input name="salary" value={formData.salary} onChange={handleManualChange} placeholder="e.g. Unpaid / $500 monthly" className={getInputClass(formData.salary)} />
            </div>
            <div className="space-y-1.5">
            ...
            <div className="sm:col-span-2 space-y-1.5">
              <label className="text-xs sm:text-sm font-bold text-slate-800">About Company</label>
              <textarea name="about_company" value={formData.about_company} onChange={handleManualChange} rows={3} placeholder="Paste the 'About Us' section from the job post..." className={getInputClass(formData.about_company)}></textarea>
            </div>
            <div className="sm:col-span-2 space-y-1.5">
        ...
        {activeTab === 'upload' && (
          <div className="animate-in zoom-in-95 duration-500">
            <div 
              className={`relative min-h-[250px] sm:min-h-[300px] flex flex-col items-center justify-center border-2 border-dashed rounded-2xl transition-all cursor-pointer overflow-hidden ${selectedFile ? 'border-blue-500 bg-slate-50' : 'border-slate-200 bg-slate-50 hover:bg-slate-100'}`}
            ...
            >
              {selectedFile ? (
                <div className="relative w-full flex flex-col items-center p-4 sm:p-6">
                  ...
                  <p className="text-blue-600 font-bold text-sm sm:text-lg text-center break-all px-4">{selectedFile.name}</p>
